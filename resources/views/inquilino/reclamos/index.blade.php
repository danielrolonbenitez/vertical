    ...
    <div class="tab-pane active">
      ...
      <table class="table table-hover">
          <thead>
            <tr class="filters">
              <th class="col-md-1">
                <a href="{{ route('reclamos.index') }}?sortby=reclamos.id&order=<?php echo $order ?><?php if (isset($_GET["search"])) { echo "&search=".$_GET["search"]; } ?>">Numero</a>
              </th>
              <th>
                <a href="{{ route('reclamos.index') }}?sortby=titulo&order=<?php echo $order ?><?php if (isset($_GET["search"])) { echo "&search=".$_GET["search"]; } ?>">Titulo</a>
              </th>
              <th>
                <a >Unidad</a>
              </th>
          ...
          <tbody>
            @if (count($reclamos)!=0)
              ...
              <tr>
                <td class="col-md-1"><a href="{{ route('reclamos.show', $reclamo->id) }}">{{ $reclamo->id }}</a></td>
