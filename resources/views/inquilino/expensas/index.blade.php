      ...
      <div class="panel-body col-xs-12">
        <table class="table table-hover">
          <thead>
            <tr>
              <th class="col-md-1">
                <a href="{{ route('expensas.index') }}?sortby=id&amp;order=<?php echo $order ?><?php if (isset($_GET["search"])) { echo "&amp;search=".$_GET["search"]; } ?>">ID</a>
              </th>
              <th>
                <a href="{{ route('expensas.index') }}?sortby=created_at&amp;order=<?php echo $order ?><?php if (isset($_GET["search"])) { echo "&amp;search=".$_GET["search"]; } ?>">Periodo</a>
              </th>
              <th>
                <a href="{{ route('expensas.index') }}?sortby=vencimiento&amp;order=<?php echo $order ?><?php if (isset($_GET["search"])) { echo "&amp;search=".$_GET["search"]; } ?>">Vencimiento</a>                 
              </th>
              <th>
                <a>Operaciones</a>                    
              </th>
...
<?php $search="" ?>
@endif
{!! $expensas->appends(['sortby' => $sortby, 'order' => $order, 'search' => $search])->render() !!}
@stop